def findMedianSortedArrays(nums1: list, nums2: list) -> float:
    ...
    if (len1 + len2) % 2 != 0:
        return get_kth(nums1, nums2, 0, len1 - 1, 0, len2 - 1, (len1 + len2) // 2)
    else:
        # when total length is even, the median is the average of the middle 2
        middle1 = get_kth(nums1, nums2, 0, len1 - 1, 0, len2 - 1, (len1 + len2) // 2)
        middle2 = get_kth(
            nums1, nums2, 0, len1 - 1, 0, len2 - 1, (len1 + len2) // 2 - 1
        )
...
def get_kth(nums1, nums2, start1, end1, start2, end2, k):
    ...
    # if sum of two median's indices is smaller than k
